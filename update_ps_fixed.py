...
def update_app_js():
    with open('/Users/satya/Desktop/code-a-thon/app.js', 'r') as f:
        content = f.read()

    new_problem_statements = """const PROBLEM_STATEMENTS = [
  {
    id: 'PS1',
    track: 1,
...
• Scheduled Research Digest: Allow recurring queries that run automatically and notify users of new papers.`
    ...
    new_show_details = """  function showProblemDetails(p) {
    ...
    # Replace PROBLEM_STATEMENTS exactly
    # We will find the PROBLEM_STATEMENTS array block.
    start_ps = content.find("const PROBLEM_STATEMENTS = [")
    if start_ps != -1:
        end_ps = content.find("];", start_ps) + 2
        content = content[:start_ps] + new_problem_statements + content[end_ps:]

    # Replace showProblemDetails
    start_def = content.find("  function showProblemDetails(p) {")
    if start_def != -1:
        end_def = content.find("    `;", start_def) + 6
        content = content[:start_def] + new_show_details + content[end_def:]

    with open('/Users/satya/Desktop/code-a-thon/app.js', 'w') as f:
        f.write(content)
